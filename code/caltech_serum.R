...
# Read in data
feature_table <- read_csv("data/mice_caltech/serum/gnps_quant_serum.csv")
metadata <- read_csv("data/mice_caltech/serum/meta_serum.csv")
sample_order <- read.csv("data/mice_caltech/serum/sequence_serum.csv")
annotations <- read.delim("data/mice_caltech/serum/fbmn/nf_output/library/merged_results_with_gnps.tsv") %>%
...
canopus <- read_tsv("data/mice_caltech/serum/canopus_serum.tsv") %>% dplyr::select(1,3,5,7,9)
...
#write_csv(x = info_feature_complete, file = "serum_annotations.csv")

# Extra bile acids libraries and rev cosine
ba_ipsita <- read_csv("data/mice_caltech/serum/merged_Bile_acid_classic_networking.csv") %>% 
  dplyr::filter(SpectrumID %in% annotations$SpectrumID)
ba_rev <- read_tsv("data/mice_caltech/serum/candidate_BA_with_best_annotations.tsv") %>% 
...
# Problem in acquisition at the end of the run. Samples have been re-acquired
...
data_filter <- data %>% dplyr::filter(!(SampleID %in% sample_rerun$SampleID1)) %>%
  dplyr::filter(!(SampleID %in% c("Pool_qc_serum_17", "Pool_qc_serum_18", "Pool_qc_serum_19", "Pool_qc_serum_20",
                                  "Pool_qc_serum_21", "Pool_qc_serum_22", "Pool_qc_serum_23", "sixmix_60")))
...
data_TIC %>% dplyr::filter(str_detect(pattern = "3xtg|5xfad", File.Name)) %>% 
  ggscatter("Order", "TIC", add = "reg.line") + ylim(0, 1e9) +
  stat_cor() # possible outliers (S3xtg_67, S3xtg_125, S3xtg_4)
...
data_TIC %>% dplyr::filter(str_detect(pattern = "sixmix", File.Name)) %>% 
  ggscatter("Order", "TIC", add = "reg.line") + ylim(0, 3e8) +
  stat_cor()
...
table_IS <- data_filter %>% column_to_rownames("SampleID") %>% t() %>% as.data.frame() %>% rownames_to_column("ID") %>% 
  ...
  rownames_to_column("SampleID") %>% dplyr::filter(!(str_detect(SampleID, "srm|sixmix|Blank|blank|Pool|pool"))) %>%
  dplyr::select(SampleID, `6066`) %>% left_join(metadata_metabolomics)
...
# problem with wet lab --> reconstituted part of the samples with solvent with no internal standard

table_IS_filter <- table_IS %>% dplyr::filter(Sulfadimethoxine > 10000000) 

table_IS_filter %>% ggscatter(x = "Order", y = "Sulfadimethoxine", add = c("reg.line")) +
  stat_cor()

table_IS_filter %>% ggbarplot(x = "Order", y = "Sulfadimethoxine", xlab = "Run Order", 
                       ylab = "Peak Area Sulfadimethoxine", title = "Internal Standard Acquisition") +
  geom_hline(yintercept = mean(table_IS_filter$Sulfadimethoxine, na.rm = TRUE), linetype = "dashed", color = "blue")

cv_is <- sd(table_IS_filter$Sulfadimethoxine)/mean(table_IS_filter$Sulfadimethoxine)
...
feature_to_rt <- info_feature_complete %>% dplyr::filter(RT < 0.2 | RT > 8) %>%
  ...
  dplyr::filter(!(Feature %in% c(20160)))
...
PCA_raw_plot <- PCA_raw_scores %>%
  ggscatter(x = "PC1", y = "PC2", color = i, alpha = 0.6,
            title = paste("PCA - Serum Metabolome", i, sep = " "),
...
PCA_plot <- PCA_whole_scores %>%
  ggscatter(x = "PC1", y = "PC2", color = i, alpha = 0.6,
            title = "Serum", palette = c("#009392", "#F0746E"),
...
#ggsave(plot = PCA_plot, filename = "serum_pca_metabolome.svg", device = "svg", dpi = "retina", height = 2, width = 2)
...
#write_csv(x = data_sample, file = "serum_metabolome.csv")
#write_csv(x = data_sample_clr, file = "serum_metabolome_rclr.csv")
#write_csv(x = metadata_metabolomics %>% dplyr::filter(SampleID %in% data_sample$SampleID), file = "serum_metadata.csv")


# Separate animal by study, colonization, strain, and sex
...
data_3xtg <- data_sample %>% dplyr::filter(SampleID %in% sample_3xtg$SampleID) %>%
  dplyr::filter(!(SampleID %in% c("S3xtg_116", "S3xtg_117", "S3xtg_91", "S3xtg_92", 
                                  "S3xtg_93", "S3xtg_94", "S3xtg_95")))
...
for (i in c("box", "host_age", "sex", "Plate",  "Colonized", "Strain")) {
  ...
  PCA_plot <- PCA_whole_scores %>%
    ggscatter(x = "PC1", y = "PC2", color = i, alpha = 0.6,
              title = paste("PCA - Serum 3xTG", i, sep = " "),
              xlab = paste("PC1 (", round(PCA_whole$prop_expl_var$X[1]*100, digits = 1),"%)", sep = ""), 
              ylab = paste("PC2 (", round(PCA_whole$prop_expl_var$X[2]*100, digits = 1),"%)", sep = ""),
              ggtheme = theme_classic()) +
    geom_point(data = PCA_whole_scores %>% group_by((!!sym(i))) %>% 
...
PLSDA_3xtg_strain_plot <- PLSDA_3xtg_strain_scores %>%
  ggscatter(x = "comp1", y = "comp2", color = "Strain", alpha = 0.6, title = "PLSDA - Serum 3xTG - Strain",
...
PLSDA_3xtg_colonized_plot <- PLSDA_3xtg_colonized_scores %>%
  ggscatter(x = "comp1", y = "comp2", color = "Colonized", alpha = 0.6, title = "PLSDA - Serum 3xTG - Colonized",
...
PLSDA_3xtg_sex_plot <- PLSDA_3xtg_sex_scores %>%
  ggscatter(x = "comp1", y = "comp2", color = "sex", alpha = 0.6, title = "PLSDA - Serum 3xTG - Sex",
...
#write_csv(x = VIPs_3xtg_strain_no_sex, file = "3xtg_serum_plsda_vip_all_samples_genotype.csv")
#write_csv(x = VIPs_3xtg_colo_no_sex, file = "3xtg_serum_plsda_vip_all_samples_colonization.csv")
#write_csv(x = VIPs_3xtg_colo_strain_clean, file = "3xtg_serum_plsda_vip_all_samples_genotype_colonization_intersection.csv")
...
for (i in c("box", "host_age", "sex", "Plate", "Strain")) {
  ...
  PCA_plot <- PCA_3xtg_gf_scores %>%
    ggscatter(x = "PC1", y = "PC2", color = i, alpha = 0.6,
              title = paste("PCA - Serum 3xTG - GF", i, sep = " "),
...
for (i in c("box", "host_age", "sex", "Plate", "Strain")) {
  ...
  PCA_plot <- PCA_3xtg_spf_scores %>%
    ggscatter(x = "PC1", y = "PC2", color = i, alpha = 0.6,
              title = paste("PCA - Serum 3xTG SPF", i, sep = " "),
...
PLSDA_3xtg_gf_strain_plot <- PLSDA_3xtg_gf_strain_scores %>%
  ggscatter(x = "comp1", y = "comp2", color = "Strain", alpha = 0.6, title = "PLSDA - Serum 3xTG GF - Strain",
...
PLSDA_3xtg_spf_strain_plot <- PLSDA_3xtg_spf_strain_scores %>%
  ggscatter(x = "comp1", y = "comp2", color = "Strain", alpha = 0.6, title = "PLSDA - Serum 3xTG SPF - Strain",
...
#            as.data.frame() %>% dplyr::mutate(Model = "3xtg_spf_strain"), file = "serum_plsda_performance_3xtg.csv")
...
#pdf(file="3xtg_serum_upset_genotype.pdf", width = 3, height = 2.5)
#strain_3xtg
#dev.off()
...
#write_csv(x = features_3xtg_mut, file = "3xtg_serum_genotype_mut.csv")
#write_csv(x = features_3xtg_wt, file = "3xtg_serum_genotype_wt.csv")
...
for (i in c("box", "host_age", "Plate", "Colonized", "sex")) {
  ...
  PCA_plot <- PCA_3xtg_mut_scores %>%
    ggscatter(x = "PC1", y = "PC2", color = i, alpha = 0.6,
              title = paste("PCA - Serum 3xTG - Mut", i, sep = " "),
...
for (i in c("box", "host_age", "Plate", "Colonized", "sex")) {
  ...
  PCA_plot <- PCA_3xtg_wt_scores %>%
    ggscatter(x = "PC1", y = "PC2", color = i, alpha = 0.6,
              title = paste("PCA - Serum 3xTG - WT", i, sep = " "),
...
PLSDA_3xtg_mut_colo <- mixOmics::plsda(data_3xtg_mut_clr %>% select_at(vars(-one_of(nearZeroVar(., names = TRUE)))),
                                         PCA_3xtg_mut_scores$Colonized, ncomp = 2, scale = TRUE)
PLSDA_3xtg_mut_colo_scores <- data.frame(PLSDA_3xtg_mut_colo$variates$X) %>% 
  rownames_to_column("SampleID") %>% left_join(metadata_metabolomics)

PLSDA_3xtg_mut_colo_plot <- PLSDA_3xtg_mut_colo_scores %>%
  ggscatter(x = "comp1", y = "comp2", color = "Colonized", alpha = 0.6, title = "PLSDA - Serum 3xTG Mut - Colonization",
...
PLSDA_3xtg_wt_colo <- mixOmics::plsda(data_3xtg_wt_clr %>% select_at(vars(-one_of(nearZeroVar(., names = TRUE)))), 
                                        PCA_3xtg_wt_scores$Colonized, ncomp = 4, scale = TRUE)
PLSDA_3xtg_wt_colo_scores <- data.frame(PLSDA_3xtg_wt_colo$variates$X) %>% 
  rownames_to_column("SampleID") %>% left_join(metadata_metabolomics)

PLSDA_3xtg_wt_colo_plot <- PLSDA_3xtg_wt_colo_scores %>%
  ggscatter(x = "comp1", y = "comp2", color = "Colonized", alpha = 0.6, title = "PLSDA - Serum 3xTG WT - Colonization",
...
#pdf(file="3xtg_serum_upset_colonization.pdf", width = 3, height = 2.5)
#colonization_3xtg
#dev.off()
...
#write_csv(x = features_3xtg_spf, file = "3xtg_serum_colonization_spf.csv")
#write_csv(x = features_3xtg_gf, file = "3xtg_serum_colonization_gf.csv")


############
# Compare 3xtg male v wt male to 3xtg female v wt female to see strain and microbiome effect within sex
############
...
for (i in c("box", "host_age", "Plate", "Strain", "Colonized")) {
  ...
  PCA_plot <- PCA_3xtg_male_scores %>%
    ggscatter(x = "PC1", y = "PC2", color = i, alpha = 0.6,
              title = paste("PCA - Serum 3xTG Male", i, sep = " "),
...
for (i in c("box", "host_age", "Plate", "Strain", "Colonized")) {
  ...
  PCA_plot <- PCA_3xtg_female_scores %>%
    ggscatter(x = "PC1", y = "PC2", color = i, alpha = 0.6,
              title = paste("PCA - Serum 3xTG Female", i, sep = " "),
...
PLSDA_3xtg_male_strain_plot <- PLSDA_3xtg_male_strain_scores %>%
  ggscatter(x = "comp1", y = "comp2", color = "Strain", alpha = 0.6, title = "PLSDA - Serum 3xTG Male - Strain",
...
PLSDA_3xtg_male_colo_plot <- PLSDA_3xtg_male_colo_scores %>%
  ggscatter(x = "comp1", y = "comp2", color = "Colonized", alpha = 0.6, title = "PLSDA - Serum 3xTG Male - Colonized",
...
PLSDA_3xtg_female_strain_plot <- PLSDA_3xtg_female_strain_scores %>%
  ggscatter(x = "comp1", y = "comp2", color = "Strain", alpha = 0.6, title = "PLSDA - Serum 3xTG Female - Strain",
...
PLSDA_3xtg_female_colo_plot <- PLSDA_3xtg_female_colo_scores %>%
  ggscatter(x = "comp1", y = "comp2", color = "Colonized", alpha = 0.6, title = "PLSDA - Serum 3xTG Female - Colonized",
...
###################
# Combine microbiome and genotype effect models to see intersection
###################
...
#pdf(file="3xtg_serum_upset_genotype_colonization.pdf", width = 8, height = 4.5)
#intersection_3xtg
#dev.off()
...
exclusive_3xtg_serum <- rbind(combined_unique_3xtg_3xtg_spf, combined_unique_3xtg_wt_spf)
...
plot_ratio_serum_3xtg <- data_3xtg_vip_spf %>% 
  dplyr::filter(Colonized == "SPF") %>% 
...
plot_ratio_serum_3xtg_sex <- data_3xtg_vip_spf %>% 
  dplyr::filter(Colonized == "SPF") %>% 
  dplyr::mutate(Strain = factor(Strain, levels = c("WT", "Mut"))) %>%
  ggboxplot(y = "Ratio", x = "Strain", add = "jitter", ylab = "Ln(WT/Mut)",
            color = "sex", alpha = 0.6, legend = "none",
            palette = c("#287DAB", "#E5BF86"), xlab = "Serum",
            title = "Differential features from PLS-DA models") +
  theme(plot.title = element_text(size = 8),axis.title = element_text(size = 7),
        axis.text = element_text(size = 6))

#ggsave(plot = plot_ratio_serum_3xtg, filename = "3xtg_serum_ratio.svg", device = "svg", dpi = "retina", width = 1.5, height = 2.5)
#write_csv(x = data_3xtg_vip_spf, file = "serum_ratio_3xtg.csv")
...
# Extract for MN
serum_3xtg_interest <- VIPs_3xtg_spf_strain_Load %>% 
  ...
  left_join(exclusive_3xtg_serum %>% dplyr::select(ID, Group)) %>% arrange(desc(VIP_mean))

colnames(serum_3xtg_interest) <- c("ID", "VIP1", "SPF_genotype", "VIP2", "GF_genotype",
                                   "VIP3", "3xTG_colonization", "VIP4", "WT_colonization", 
                                   "VIP_mean", "Exclusivity")

serum_3xtg_interest <- serum_3xtg_interest %>%   
...
#write_csv(x = serum_3xtg_interest, file = "3xtg_serum_features_info_mn.csv")

# BAs
ba_new <- serum_3xtg_interest %>% dplyr::filter(!is.na(SpectrumID)) %>%
  dplyr::left_join(ba_ipsita %>% dplyr::select(SpectrumID, synlib_compound_name))


# Plot some carnitines
carnitines <- info_feature_complete %>% dplyr::filter(str_detect(pattern = "carnitine", Compound_Name))

val_car <- data_3xtg %>% dplyr::select(SampleID, carnitines$Feature) %>%
  left_join(metadata_metabolomics) %>% dplyr::filter(Colonized == "SPF") %>%
  pivot_longer(cols = c(2:18), names_to = "Feature", values_to = "Peak Area") %>%
  left_join(carnitines %>% dplyr::select(Feature, Compound_Name)) %>%
  dplyr::mutate(Strain = factor(Strain, levels = c("WT", "Mut"))) %>%
  dplyr::filter(Feature == 2408) %>% dplyr::mutate(LogPeak = log(`Peak Area`)) %>%
  ggboxplot(x = "Strain", y = "Peak Area", add = "jitter",
            add.params = list(color = "Strain", alpha = 0.6), legend = "none",
            palette = c("#E69A8DFF", "#5F4B8BFF"), xlab = "Serum",
            title = "Valeryl-carnitine") +
  theme(plot.title = element_text(size = 8),axis.title = element_text(size = 7),
        axis.text = element_text(size = 6)) +
  stat_compare_means()

prop_car <- data_3xtg %>% dplyr::select(SampleID, carnitines$Feature) %>%
  left_join(metadata_metabolomics) %>% dplyr::filter(Colonized == "SPF") %>%
  pivot_longer(cols = c(2:18), names_to = "Feature", values_to = "Peak Area") %>%
  left_join(carnitines %>% dplyr::select(Feature, Compound_Name)) %>%
  dplyr::mutate(Strain = factor(Strain, levels = c("WT", "Mut"))) %>%
  dplyr::filter(Feature == 440) %>% dplyr::mutate(LogPeak = log(`Peak Area`)) %>%
  ggboxplot(x = "Strain", y = "Peak Area", add = "jitter",
            add.params = list(color = "Strain", alpha = 0.6), legend = "none",
            palette = c("#E69A8DFF", "#5F4B8BFF"), xlab = "Serum",
            title = "Propyonyl-carnitine") +
  theme(plot.title = element_text(size = 8),axis.title = element_text(size = 7),
        axis.text = element_text(size = 6)) +
  stat_compare_means()

serum_car <- ggarrange(prop_car, val_car, nrow = 1)

#ggsave(plot = serum_car, filename = "serum_car.svg", device = "svg", dpi = "retina", height = 1.5, width = 2)

un_car1 <- data_3xtg %>% dplyr::select(SampleID, `4173`) %>%
  left_join(metadata_metabolomics) %>% dplyr::filter(Colonized == "SPF") %>%
  dplyr::mutate(Strain = factor(Strain, levels = c("WT", "Mut"))) %>%
  dplyr::rename("Peak Area" = `4173`) %>%
  ggboxplot(x = "Strain", y = "Peak Area", add = "jitter",
            add.params = list(color = "Strain", alpha = 0.6), legend = "none",
            palette = c("#E69A8DFF", "#5F4B8BFF"), xlab = "Serum",
            title = "Unknown Carnitine - m/z 294.168") +
  theme(plot.title = element_text(size = 8),axis.title = element_text(size = 7),
        axis.text = element_text(size = 6)) +
  stat_compare_means()

un_car2 <- data_3xtg %>% dplyr::select(SampleID, `3078`) %>%
  left_join(metadata_metabolomics) %>% dplyr::filter(Colonized == "SPF") %>%
  dplyr::mutate(Strain = factor(Strain, levels = c("WT", "Mut"))) %>%
  dplyr::rename("Peak Area" = `3078`) %>%
  ggboxplot(x = "Strain", y = "Peak Area", add = "jitter",
            add.params = list(color = "Strain", alpha = 0.6), legend = "none",
            palette = c("#E69A8DFF", "#5F4B8BFF"), xlab = "Serum",
            title = "Unknown Carnitine - m/z 280.153") +
  theme(plot.title = element_text(size = 8),axis.title = element_text(size = 7),
        axis.text = element_text(size = 6)) +
  stat_compare_means()

un_car3 <- data_3xtg %>% dplyr::select(SampleID, `2821`) %>%
  left_join(metadata_metabolomics) %>% dplyr::filter(Colonized == "SPF") %>%
  dplyr::mutate(Strain = factor(Strain, levels = c("WT", "Mut"))) %>%
  dplyr::rename("Peak Area" = `2821`) %>%
  ggboxplot(x = "Strain", y = "Peak Area", add = "jitter",
            add.params = list(color = "Strain", alpha = 0.6), legend = "none",
            palette = c("#E69A8DFF", "#5F4B8BFF"), xlab = "Serum",
            title = "Unknown Carnitine - m/z 266.137") +
  theme(plot.title = element_text(size = 8),axis.title = element_text(size = 7),
        axis.text = element_text(size = 6)) +
  stat_compare_means()

serum_car_un <- ggarrange(un_car1, un_car2, un_car3, nrow = 1)

#ggsave(plot = serum_car_un, filename = "serum_car_un.svg", device = "svg", dpi = "retina", height = 1.5, width = 2)
...
VIPs_3xtg_serum_eff <- VIPs_3xtg_spf_strain_Load_filter %>%
  left_join(cliffs_delta_results, by = c("ID" = "FeatureID")) %>%
  dplyr::mutate(Compound_Name = tolower(Compound_Name))

write_csv(x = VIPs_3xtg_serum_eff, file = "data/mice_caltech/serum/vip_3xtg_spf.csv")

# Read cleaned and manually inspected file 
VIPs_3xtg_serum_eff_final <- read_csv("data/mice_caltech/serum/vip_3xtg_spf_manual.csv") %>%
...
df_prepared <- VIPs_3xtg_serum_eff_final %>%
...
combined_plot <- p_vip + p_forest + 
  ...
  plot_annotation(title = '3xTg Serum',
                  theme = theme(plot.title = element_text(size = 8, face = "bold")))

#ggsave(plot = combined_plot, filename = "figure3c_serum.svg", device = "svg", dpi = "retina", height = 3.5, width = 2.5)
...
data_5xfad <- data_sample %>% dplyr::filter(SampleID %in% sample_5xfad$SampleID) %>%
  dplyr::filter(!(SampleID %in% c("S5xfad_20", "S5xfad_98", "S5xfad_130", 
                                  "S5xfad_110", "S5xfad_113")))
...
for (i in c("box", "host_age", "sex", "Plate",  "Colonized", "Strain")) {
  ...
  PCA_plot <- PCA_whole_scores %>%
    ggscatter(x = "PC1", y = "PC2", color = i, alpha = 0.6,
              title = paste("PCA - Serum 5xFAD", i, sep = " "),
              xlab = paste("PC1 (", round(PCA_whole$prop_expl_var$X[1]*100, digits = 1),"%)", sep = ""), 
              ylab = paste("PC2 (", round(PCA_whole$prop_expl_var$X[2]*100, digits = 1),"%)", sep = ""),
              ggtheme = theme_classic()) +
    geom_point(data = PCA_whole_scores %>% group_by((!!sym(i))) %>% 
...
PLSDA_5xfad_strain_plot <- PLSDA_5xfad_strain_scores %>%
  ggscatter(x = "comp1", y = "comp2", color = "Strain", alpha = 0.6, title = "PLSDA - Serum 5xFAD - Strain",
...
PLSDA_5xfad_colonized_plot <- PLSDA_5xfad_colonized_scores %>%
  ggscatter(x = "comp1", y = "comp2", color = "Colonized", alpha = 0.6, title = "PLSDA - Serum 5xFAD - Colonized", 
...
PLSDA_5xfad_sex_plot <- PLSDA_5xfad_sex_scores %>%
  ggscatter(x = "comp1", y = "comp2", color = "sex", alpha = 0.6, title = "PLSDA - Serum 5xFAD - Sex",
...
#write_csv(x = VIPs_5xfad_strain_no_sex, file = "5xfad_serum_plsda_vip_all_samples_genotype.csv")
#write_csv(x = VIPs_5xfad_colo_no_sex, file = "5xfad_serum_plsda_vip_all_samples_colonization.csv")
#write_csv(x = VIPs_5xfad_colo_strain_clean, file = "5xfad_serum_plsda_vip_all_samples_genotype_colonization_intersection.csv")


############
# Compare 5xfad gf v wt gf to 5xfad spf v wt spf to see strain effect
############
data_5xfad_gf <- data_sample %>% dplyr::filter(SampleID %in% sample_5xfad$SampleID) %>%
  dplyr::filter(SampleID %in% sample_gf$SampleID) %>%
  dplyr::filter(!(SampleID %in% c("S5xfad_20", "S5xfad_98", "S5xfad_130", "S5xfad_110", "S5xfad_113")))

data_5xfad_spf <- data_sample %>% dplyr::filter(SampleID %in% sample_5xfad$SampleID) %>%
  dplyr::filter(SampleID %in% sample_spf$SampleID) %>%
  dplyr::filter(!(SampleID %in% c("S5xfad_20", "S5xfad_98", "S5xfad_130", "S5xfad_110", "S5xfad_113")))
...
for (i in c("box", "host_age", "sex", "Plate", "Strain")) {
  ...
  PCA_plot <- PCA_5xfad_gf_scores %>%
    ggscatter(x = "PC1", y = "PC2", color = i, alpha = 0.6,
              title = paste("PCA - Serum 5xFAD - GF", i, sep = " "),
...
for (i in c("box", "host_age", "sex", "Plate", "Strain")) {
  ...
  PCA_plot <- PCA_5xfad_spf_scores %>%
    ggscatter(x = "PC1", y = "PC2", color = i, alpha = 0.6,
              title = paste("PCA - Serum 5xFAD SPF", i, sep = " "),
...
PLSDA_5xfad_gf_strain <- mixOmics::plsda(data_5xfad_gf_clr %>% select_at(vars(-one_of(nearZeroVar(., names = TRUE)))),
                                         PCA_5xfad_gf_scores$Strain, ncomp = 2, scale = TRUE)
...
PLSDA_5xfad_gf_strain_plot <- PLSDA_5xfad_gf_strain_scores %>%
  ggscatter(x = "comp1", y = "comp2", color = "Strain", alpha = 0.6, title = "PLSDA - Serum 5xFAD GF - Strain",
...
PLSDA_5xfad_spf_strain_plot <- PLSDA_5xfad_spf_strain_scores %>%
  ggscatter(x = "comp1", y = "comp2", color = "Strain", alpha = 0.6, title = "PLSDA - Serum 5xFAD SPF - Strain",
...
#            as.data.frame() %>% dplyr::mutate(Model = "5xfad_spf_strain"), file = "serum_plsda_performance_5xfad.csv")
...
strain_5xfad <- UpSetR::upset(fromList(list_5xfad_strain), nsets = 4, nintersects = 8, 
                              point.size = 1.5, line.size = 1, text.scale = 1, keep.order = TRUE,  
...
#pdf(file="5xfad_serum_upset_genotype.pdf", width = 3, height = 2.5)
#strain_5xfad
#dev.off()
...
#write_csv(x = features_5xfad_mut, file = "5xfad_serum_genotype_mut.csv")
#write_csv(x = features_5xfad_wt, file = "5xfad_serum_genotype_wt.csv")


############
# Compare 5xfad gf v 5xfad spf to wt gf v wt spf to see microbiome effect
############
data_5xfad_mut <- data_sample %>% dplyr::filter(SampleID %in% sample_5xfad$SampleID) %>%
  dplyr::filter(SampleID %in% sample_mut$SampleID) %>%
  dplyr::filter(!(SampleID %in% c("S5xfad_20", "S5xfad_98", "S5xfad_130", "S5xfad_110", "S5xfad_113")))

data_5xfad_wt <- data_sample %>% dplyr::filter(SampleID %in% sample_5xfad$SampleID) %>%
  dplyr::filter(SampleID %in% sample_wt$SampleID) %>%
  dplyr::filter(!(SampleID %in% c("S5xfad_20", "S5xfad_98", "S5xfad_130", "S5xfad_110", "S5xfad_113")))
...
for (i in c("box", "host_age", "Plate", "Colonized", "sex")) {
  ...
  PCA_plot <- PCA_5xfad_mut_scores %>%
    ggscatter(x = "PC1", y = "PC2", color = i, alpha = 0.6,
              title = paste("PCA - Serum 5xFAD - Mut", i, sep = " "),
...
for (i in c("box", "host_age", "Plate", "Colonized", "sex")) {
  ...
  PCA_plot <- PCA_5xfad_wt_scores %>%
    ggscatter(x = "PC1", y = "PC2", color = i, alpha = 0.6,
              title = paste("PCA - Serum 5xFAD - WT", i, sep = " "),
...
PLSDA_5xfad_mut_colo <- mixOmics::plsda(data_5xfad_mut_clr %>% select_at(vars(-one_of(nearZeroVar(., names = TRUE)))),
                                          PCA_5xfad_mut_scores$Colonized, ncomp = 3, scale = TRUE)
...
PLSDA_5xfad_mut_colo_plot <- PLSDA_5xfad_mut_colo_scores %>%
  ggscatter(x = "comp1", y = "comp2", color = "Colonized", alpha = 0.6, title = "PLSDA - Serum 5xFAD Mut - Colonization",
...
PLSDA_5xfad_wt_colo <- mixOmics::plsda(data_5xfad_wt_clr %>% select_at(vars(-one_of(nearZeroVar(., names = TRUE)))),
                                         PCA_5xfad_wt_scores$Colonized, ncomp = 3, scale = TRUE)
PLSDA_5xfad_wt_colo_scores <- data.frame(PLSDA_5xfad_wt_colo$variates$X) %>% 
  rownames_to_column("SampleID") %>% left_join(metadata_metabolomics)

PLSDA_5xfad_wt_colo_plot <- PLSDA_5xfad_wt_colo_scores %>%
  ggscatter(x = "comp1", y = "comp2", color = "Colonized", alpha = 0.6, title = "PLSDA - Serum 5xFAD WT - Colonization",
...
#pdf(file="5xfad_serum_upset_colonization.pdf", width = 3, height = 2.5)
#colonization_5xfad
#dev.off()
...
#write_csv(x = features_5xfad_spf, file = "5xfad_serum_colonization_spf.csv")
#write_csv(x = features_5xfad_gf, file = "5xfad_serum_colonization_gf.csv")


############
# Compare 5xfad male v wt male to 5xfad female v wt female to see strain effect on sex
############
data_5xfad_male <- data_sample %>% dplyr::filter(SampleID %in% sample_5xfad$SampleID) %>%
  dplyr::filter(SampleID %in% sample_male$SampleID) %>%
  dplyr::filter(!(SampleID %in% c("S5xfad_20", "S5xfad_98", "S5xfad_130", "S5xfad_110", "S5xfad_113")))

data_5xfad_female <- data_sample %>% dplyr::filter(SampleID %in% sample_5xfad$SampleID) %>%
  dplyr::filter(SampleID %in% sample_female$SampleID) %>%
  dplyr::filter(!(SampleID %in% c("S5xfad_20", "S5xfad_98", "S5xfad_130", "S5xfad_110", "S5xfad_113")))
...
for (i in c("box", "host_age", "Plate", "Strain", "Colonized")) {
  ...
  PCA_plot <- PCA_5xfad_male_scores %>%
    ggscatter(x = "PC1", y = "PC2", color = i, alpha = 0.6,
              title = paste("PCA - Serum 5xFAD Male", i, sep = " "),
...
for (i in c("box", "host_age", "Plate", "Strain", "Colonized")) {
  ...
  PCA_plot <- PCA_5xfad_female_scores %>%
    ggscatter(x = "PC1", y = "PC2", color = i, alpha = 0.6,
              title = paste("PCA - Serum 5xFAD Female", i, sep = " "),
...
PLSDA_5xfad_male_strain <- mixOmics::plsda(data_5xfad_male_clr %>% select_at(vars(-one_of(nearZeroVar(., names = TRUE)))),
                                           PCA_5xfad_male_scores$Strain, ncomp = 2, scale = TRUE)
...
PLSDA_5xfad_male_strain_plot <- PLSDA_5xfad_male_strain_scores %>%
  ggscatter(x = "comp1", y = "comp2", color = "Strain", alpha = 0.6, title = "PLSDA - Serum 5xFAD Male - Strain",
...
PLSDA_5xfad_male_colo_plot <- PLSDA_5xfad_male_colo_scores %>%
  ggscatter(x = "comp1", y = "comp2", color = "Colonized", alpha = 0.6, title = "PLSDA - Serum 5xFAD Male - Colonized",
...
PLSDA_5xfad_female_strain_plot <- PLSDA_5xfad_female_strain_scores %>%
  ggscatter(x = "comp1", y = "comp2", color = "Strain", alpha = 0.6, title = "PLSDA - Serum 5xFAD Female - Strain",
...
PLSDA_5xfad_female_colo_plot <- PLSDA_5xfad_female_colo_scores %>%
  ggscatter(x = "comp1", y = "comp2", color = "Strain", alpha = 0.6, title = "PLSDA - Serum 5xFAD Female - Colonized",
...
#pdf(file="5xfad_serum_upset_genotype_colonization.pdf", width = 6.5, height = 4.5)
#intersection_5xfad
#dev.off()
...
# Extract feature of interest and check difference between strains colonized
exclusive_5xfad_serum <-  rbind(combined_unique_5xfad_5xfad_spf, combined_unique_5xfad_wt_spf)

data_5xfad_feat <- data_sample %>% dplyr::filter(SampleID %in% sample_5xfad$SampleID) %>%
  dplyr::filter(!(SampleID %in% c("S5xfad_20", "S5xfad_98", "S5xfad_130", "S5xfad_110", "S5xfad_113"))) %>%
...
data_5xfad_vip_spf <- data_sample %>% dplyr::filter(SampleID %in% sample_5xfad$SampleID) %>%
  dplyr::filter(!(SampleID %in% c("S5xfad_20", "S5xfad_98", "S5xfad_130", "S5xfad_110", "S5xfad_113"))) %>%
...
plot_ratio_serum_5xfad <- data_5xfad_vip_spf %>% 
  dplyr::filter(Colonized == "SPF") %>% 
  dplyr::mutate(Strain = factor(Strain, levels = c("WT", "Mut"))) %>%
  ggboxplot(y = "Ratio", x = "Strain", add = "jitter", ylab = "Ln(WT/Mut)",
            add.params = list(color = "Strain", alpha = 0.6), legend = "none",
...
#ggsave(plot = plot_ratio_serum_5xfad, filename = "5xfad_serum_ratio.svg", device = "svg", dpi = "retina", width = 1.5, height = 2.5)
#write_csv(x = data_5xfad_vip_spf, file = "serum_ratio_5xfad.csv")
...
# Extract for MN
serum_5xfad_interest <- VIPs_5xfad_spf_strain_Load %>% 
  dplyr::select(ID, comp1, GroupContrib) %>% 
  ...
  left_join(VIPs_5xfad_wt_colo_Load %>% 
              dplyr::select(ID, comp1, GroupContrib), by = c("ID" = "ID")) %>% 
  dplyr::mutate(VIP_mean = rowMeans(dplyr::select(., comp1.x, comp1.x.x, comp1.y, comp1.y.y), na.rm = TRUE)) %>%
  left_join(exclusive_5xfad_serum %>% dplyr::select(ID, Group)) %>% arrange(desc(VIP_mean))

colnames(serum_5xfad_interest) <- c("ID", "VIP1", "SPF_genotype", "VIP2", "GF_genotype",
                                   "VIP3", "5xFAD_colonization", "VIP4", "WT_colonization", 
                                   "VIP_mean", "Exclusivity")

serum_5xfad_interest <- serum_5xfad_interest %>%   
  dplyr::mutate(Genotype = case_when(SPF_genotype == GF_genotype ~ "Yes",
                                     TRUE ~ "No")) %>%
...
#write_csv(x = serum_5xfad_interest, file = "5xfad_serum_features_info_mn.csv")


###########
# Check overlapping between 3xTG and 5xFAD
###########
overlap_serum <- serum_3xtg_interest %>% 
  inner_join(serum_5xfad_interest, by = "ID") %>%
...
data_3xtg_join <- data_3xtg %>%
  ...
  dplyr::mutate(sample_name = gsub(".Serum", "", sample_name)) %>% dplyr::select(-SampleID)

#write_csv(x = data_3xtg_join, file = "serum_3xtg_metabolomics_join.csv")

data_5xfad_join <- data_5xfad %>%
  dplyr::left_join(metadata_metabolomics %>% dplyr::select(SampleID, sample_name)) %>%
  dplyr::relocate(sample_name, .after=SampleID) %>% 
  dplyr::mutate(sample_name = gsub(".Serum", "", sample_name)) %>% dplyr::select(-SampleID)

#write_csv(x = data_5xfad_join, file = "serum_5xfad_metabolomics_join.csv")
...
dda <- Spectra("data/mice_caltech/serum/gnps_serum.mgf", source = MsBackendMgf())
...
#export(dda_filtered, MsBackendMgf(), file = "Serum_VIPs_microbemasst.mgf", exportTitle = FALSE)
