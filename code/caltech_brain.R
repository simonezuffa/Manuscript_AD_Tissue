...
# Read in data
feature_table <- read_csv("data/mice_caltech/brain/gnps_quant_brain.csv")
metadata <- read_csv("data/mice_caltech/brain/meta_brain.csv")
sample_order <- read.csv("data/mice_caltech/brain/sequence_brain.csv")
annotations <- read.delim("data/mice_caltech/brain/fbmn/nf_output/library/merged_results_with_gnps.tsv") %>%
...
canopus <- read_tsv("data/mice_caltech/brain/canopus_brain.tsv") %>% dplyr::select(1,3,5,7,9)
...
#write_csv(x = info_feature_complete, file = "brain_annotations.csv")

# Extra bile acids libraries and rev cosine
ba_ipsita <- read_csv("data/mice_caltech/brain/merged_Bile_acid_classic_networking.csv") %>% 
  dplyr::filter(SpectrumID %in% annotations$SpectrumID)
ba_rev <- read_tsv("data/mice_caltech/brain/candidate_BA_with_best_annotations.tsv") %>% 
...
data_TIC %>% dplyr::filter(str_detect(pattern = "3xtg|5xfad", File.Name)) %>% 
  ggscatter("Order", "TIC", add = "reg.line") + ylim(0, 7.5e8) +
  stat_cor() # two possible outliers (B3xtg_1, B5xfad_68)
...
data_TIC %>% dplyr::filter(str_detect(pattern = "Pool", File.Name)) %>% 
  ggscatter("Order", "TIC", add = "reg.line") + ylim(0, 5.5e8) +
  stat_cor()
...
data_TIC %>% dplyr::filter(str_detect(pattern = "Blank", File.Name)) %>% 
  ggscatter("Order", "TIC", add = "reg.line") + ylim(0, 2.5e8) +
  stat_cor()
...
table_IS <- data %>% column_to_rownames("SampleID") %>% t() %>% as.data.frame() %>% rownames_to_column("ID") %>% 
  ...
  rownames_to_column("SampleID") %>% dplyr::filter(!(str_detect(SampleID, "srm|sixmix|Blank|Pool"))) %>%
  dplyr::select(SampleID, `1679`) %>% left_join(metadata_metabolomics)

colnames(table_IS)[2] <- "Sulfamethazine"

table_IS %>% ggscatter(x = "Order", y = "Sulfamethazine", add = c("reg.line")) + ylim(0, 6.5e6) +
  stat_cor()
...
data_pool <- data %>% dplyr::filter(str_detect(pattern = "Pool", SampleID))
data_sixmix <- data %>% dplyr::filter(str_detect(pattern = "sixmix", SampleID))
...
# CVs of the six standards are ranging between 6% to 14% --> good
...
data_sample <- data_clean3 %>% 
  ...
  dplyr::filter(!(SampleID %in% c("B3xtg_143", "B3xtg_144", "B3xtg_145", "B3xtg_76",
                                  "B3xtg_91", "B3xtg_116", "B3xtg_1", "B5xfad_25", "B5xfad_30", "B5xfad_26", "B5xfad_28", 
                                  "B5xfad_23", "B5xfad_27", "B5xfad_22", "B5xfad_20", "B5xfad_21", "B5xfad_29", "B5xfad_114",
                                  "B5xfad_1", "B5xfad_10", "B5xfad_11", "B5xfad_12", "B5xfad_13", "B5xfad_14", "B5xfad_15", 
                                  "B5xfad_16", "B5xfad_17", "B5xfad_18", "B5xfad_19", "B5xfad_24", "B5xfad_2", "B5xfad_3", 
                                  "B5xfad_4", "B5xfad_5", "B5xfad_6", "B5xfad_7", "B5xfad_8", "B5xfad_9")))
# this is to generate the figure for manuscript removing corrupted samples 
# that are removed later on in the analysis by looking at stratified groups
...
PCA_plot <- PCA_whole_scores %>%
  ggscatter(x = "PC1", y = "PC2", color = i, alpha = 0.6,
            title = "Brain", palette = c("#009392", "#F0746E"),
  ...
  theme(plot.title = element_text(size = 10), axis.title = element_text(size = 6),
        axis.text = element_text(size = 8)) + coord_fixed()

#ggsave(plot = PCA_plot, filename = "brain_pca_metabolome.svg", device = "svg", dpi = "retina", height = 2, width = 2)
...
#write_csv(x = metadata_metabolomics %>% dplyr::filter(SampleID %in% data_sample$SampleID), file = "brain_metadata.csv")
...
for (i in c("box", "host_age", "sex", "Plate",  "Colonized", "Strain")) {
  ...
  PCA_plot <- PCA_whole_scores %>%
    ggscatter(x = "PC1", y = "PC2", color = i, alpha = 0.6,
              title = paste("PCA - Brain 3xTG", i, sep = " "),
              xlab = paste("PC1 (", round(PCA_whole$prop_expl_var$X[1]*100, digits = 1),"%)", sep = ""), 
              ylab = paste("PC2 (", round(PCA_whole$prop_expl_var$X[2]*100, digits = 1),"%)", sep = ""),
              ggtheme = theme_classic()) +
    geom_point(data = PCA_whole_scores %>% group_by((!!sym(i))) %>% 
...
PLSDA_3xtg_strain <- mixOmics::plsda(data_3xtg_clr %>% select_at(vars(-one_of(nearZeroVar(., names = TRUE)))), 
                                     PCA_whole_scores$Strain, ncomp = 5, scale = TRUE)
PLSDA_3xtg_strain_scores <- data.frame(PLSDA_3xtg_strain$variates$X) %>% 
  rownames_to_column("SampleID") %>% left_join(metadata_metabolomics)

PLSDA_3xtg_strain_plot <- PLSDA_3xtg_strain_scores %>%
  ggscatter(x = "comp1", y = "comp2", color = "Strain", alpha = 0.6, title = "PLSDA - Brain 3xTG - Strain",
...
PLSDA_3xtg_colonized <- mixOmics::plsda(data_3xtg_clr %>% select_at(vars(-one_of(nearZeroVar(., names = TRUE)))), 
                                        PCA_whole_scores$Colonized, ncomp = 5, scale = TRUE)
...
PLSDA_3xtg_colonized_plot <- PLSDA_3xtg_colonized_scores %>%
  ggscatter(x = "comp1", y = "comp2", color = "Colonized", alpha = 0.6, title = "PLSDA - Brain 3xTG - Colonized",
...
PLSDA_3xtg_sex <- mixOmics::plsda(data_3xtg_clr %>% select_at(vars(-one_of(nearZeroVar(., names = TRUE)))), 
                                  PCA_whole_scores$sex, ncomp = 4, scale = TRUE)
PLSDA_3xtg_sex_scores <- data.frame(PLSDA_3xtg_sex$variates$X) %>% 
  rownames_to_column("SampleID") %>% left_join(metadata_metabolomics)

PLSDA_3xtg_sex_plot <- PLSDA_3xtg_sex_scores %>%
  ggscatter(x = "comp1", y = "comp2", color = "sex", alpha = 0.6, title = "PLSDA - Brain 3xTG - Sex",
...
#write_csv(x = VIPs_3xtg_strain_no_sex, file = "3xtg_brain_plsda_vip_all_samples_genotype.csv")
#write_csv(x = VIPs_3xtg_colo_no_sex, file = "3xtg_brain_plsda_vip_all_samples_colonization.csv")
#write_csv(x = VIPs_3xtg_colo_strain_clean, file = "3xtg_brain_plsda_vip_all_samples_genotype_colonization_intersection.csv")
...
for (i in c("box", "host_age", "sex", "Plate", "Strain")) {
  ...
  PCA_plot <- PCA_3xtg_gf_scores %>%
    ggscatter(x = "PC1", y = "PC2", color = i, alpha = 0.6,
              title = paste("PCA - Brain 3xTG - GF", i, sep = " "),
...
for (i in c("box", "host_age", "sex", "Plate", "Strain")) {
  ...
  PCA_plot <- PCA_3xtg_spf_scores %>%
    ggscatter(x = "PC1", y = "PC2", color = i, alpha = 0.6,
              title = paste("PCA - Brain 3xTG SPF", i, sep = " "),
...
PLSDA_3xtg_gf_strain_plot <- PLSDA_3xtg_gf_strain_scores %>%
  ggscatter(x = "comp1", y = "comp2", color = "Strain", alpha = 0.6, title = "PLSDA - Brain 3xTG GF - Strain",
...
PLSDA_3xtg_spf_strain <- mixOmics::plsda(data_3xtg_spf_clr %>% select_at(vars(-one_of(nearZeroVar(., names = TRUE)))),
                                         PCA_3xtg_spf_scores$Strain, ncomp = 4, scale = TRUE)
...
PLSDA_3xtg_spf_strain_plot <- PLSDA_3xtg_spf_strain_scores %>%
  ggscatter(x = "comp1", y = "comp2", color = "Strain", alpha = 0.6, title = "PLSDA - Brain 3xTG SPF - Strain",
...
#            as.data.frame() %>% dplyr::mutate(Model = "3xtg_spf_strain"), file = "brain_plsda_performance_3xtg.csv")
...
#pdf(file="3xtg_brain_genotype.pdf", width = 3, height = 2.5)
#strain_3xtg
#dev.off()
...
#write_csv(x = features_3xtg_mut, file = "3xtg_brain_genotype_3xtg.csv")
#write_csv(x = features_3xtg_wt, file = "3xtg_brain_genotype_wt.csv")
...
for (i in c("box", "host_age", "Plate", "Colonized", "sex")) {
  ...
  PCA_plot <- PCA_3xtg_mut_scores %>%
    ggscatter(x = "PC1", y = "PC2", color = i, alpha = 0.6,
              title = paste("PCA - Brain 3xTG - Mut", i, sep = " "),
...
for (i in c("box", "host_age", "Plate", "Colonized", "sex")) {
  ...
  PCA_plot <- PCA_3xtg_wt_scores %>%
    ggscatter(x = "PC1", y = "PC2", color = i, alpha = 0.6,
              title = paste("PCA - Brain 3xTG - WT", i, sep = " "),
...
PLSDA_3xtg_mut_colo <- mixOmics::plsda(data_3xtg_mut_clr %>% select_at(vars(-one_of(nearZeroVar(., names = TRUE)))), 
                                         PCA_3xtg_mut_scores$Colonized, ncomp = 4, scale = TRUE)
PLSDA_3xtg_mut_colo_scores <- data.frame(PLSDA_3xtg_mut_colo$variates$X) %>% 
  rownames_to_column("SampleID") %>% left_join(metadata_metabolomics)

PLSDA_3xtg_mut_colo_plot <- PLSDA_3xtg_mut_colo_scores %>%
  ggscatter(x = "comp1", y = "comp2", color = "Colonized", alpha = 0.6, title = "PLSDA - Brain 3xTG Mut - Colonization",
...
PLSDA_3xtg_wt_colo_plot <- PLSDA_3xtg_wt_colo_scores %>%
  ggscatter(x = "comp1", y = "comp2", color = "Colonized", alpha = 0.6, title = "PLSDA - Brain 3xTG WT - Colonization",
...
#pdf(file="3xtg_brain_colonization.pdf", width = 3, height = 2.5)
#colonization_3xtg
#dev.off()
...
#write_csv(x = features_3xtg_spf, file = "3xtg_brain_colonization_spf.csv")
#write_csv(x = features_3xtg_gf, file = "3xtg_brain_colonization_gf.csv")


############
# Compare 3xtg male v wt male to 3xtg female v wt female to see effect of sex
############
...
PLSDA_3xtg_male_strain_plot <- PLSDA_3xtg_male_strain_scores %>%
  ggscatter(x = "comp1", y = "comp2", color = "Strain", alpha = 0.6, title = "PLSDA - Brain 3xTG Male - Strain",
...
PLSDA_3xtg_male_colo_plot <- PLSDA_3xtg_male_colo_scores %>%
  ggscatter(x = "comp1", y = "comp2", color = "Colonized", alpha = 0.6, title = "PLSDA - Brain 3xTG Male - Colonized",
...
PLSDA_3xtg_female_strain_plot <- PLSDA_3xtg_female_strain_scores %>%
  ggscatter(x = "comp1", y = "comp2", color = "Strain", alpha = 0.6, title = "PLSDA - Brain 3xTG Female - Strain",
...
PLSDA_3xtg_female_colo_plot <- PLSDA_3xtg_female_colo_scores %>%
  ggscatter(x = "comp1", y = "comp2", color = "Colonized", alpha = 0.6, title = "PLSDA - Brain 3xTG Female - Colonized",
...
#pdf(file="3xtg_brain_upset_genotype_colonization.pdf", width = 6.5, height = 4.5)
#intersection_3xtg
#dev.off()
...
exclusive_3xtg_brain <- rbind(combined_unique_3xtg_3xtg_spf, combined_unique_3xtg_wt_spf)
...
plot_ratio_brain_3xtg <- data_3xtg_vip_spf %>% 
  dplyr::filter(Colonized == "SPF") %>% 
  ...
  ggboxplot(y = "Ratio", x = "Strain", add = "jitter", ylab = "Ln(WT/Mut)",
            ...
            palette = c("#E69A8DFF", "#5F4B8BFF"), xlab = "Brain",
            title = "Differential features from PLS-DA models") +
  stat_compare_means() +
  theme(plot.title = element_text(size = 8),axis.title = element_text(size = 7),
        axis.text = element_text(size = 6))

#ggsave(plot = plot_ratio_brain_3xtg, filename = "3xtg_brain_ratio.svg", device = "svg", dpi = "retina", width = 1.5, height = 2.5)
#write_csv(x = data_3xtg_vip_spf, file = "brain_ratio_3xtg.csv")
...
brain_3xtg_interest <- VIPs_3xtg_spf_strain_Load %>% 
  ...
  left_join(exclusive_3xtg_brain %>% dplyr::select(ID, Group)) %>% arrange(desc(VIP_mean))

colnames(brain_3xtg_interest) <- c("ID", "VIP1", "SPF_genotype", "VIP2", "GF_genotype",
                                   "VIP3", "3xTG_colonization", "VIP4", "WT_colonization", 
                                   "VIP_mean", "Exclusivity")

brain_3xtg_interest <- brain_3xtg_interest %>%   
...
#write_csv(x = brain_3xtg_interest, file = "3xtg_brain_features_info_mn.csv")


# Plot some carnitines
brain_car1 <- data_3xtg %>% dplyr::select(SampleID, `716`) %>%
  left_join(metadata_metabolomics) %>% dplyr::filter(Colonized == "SPF") %>%
  dplyr::mutate(Strain = factor(Strain, levels = c("WT", "Mut"))) %>%
  dplyr::rename("Peak Area" = `716`) %>%
  ggboxplot(x = "Strain", y = "Peak Area", add = "jitter",
            add.params = list(color = "Strain", alpha = 0.6), legend = "none",
            palette = c("#E69A8DFF", "#5F4B8BFF"), xlab = "Serum",
            title = "Butyryl-carnitine") +
  theme(plot.title = element_text(size = 8),axis.title = element_text(size = 7),
        axis.text = element_text(size = 6)) +
  stat_compare_means()

brain_car2 <- data_3xtg %>% dplyr::select(SampleID, `3663`) %>%
  left_join(metadata_metabolomics) %>% dplyr::filter(Colonized == "SPF") %>%
  dplyr::mutate(Strain = factor(Strain, levels = c("WT", "Mut"))) %>%
  dplyr::rename("Peak Area" = `3663`) %>%
  ggboxplot(x = "Strain", y = "Peak Area", add = "jitter",
            add.params = list(color = "Strain", alpha = 0.6), legend = "none",
            palette = c("#E69A8DFF", "#5F4B8BFF"), xlab = "Serum",
            title = "Palmitoyl-carnitine") +
  theme(plot.title = element_text(size = 8),axis.title = element_text(size = 7),
        axis.text = element_text(size = 6)) +
  stat_compare_means()

brain_car <- ggarrange(brain_car1, brain_car2, nrow = 1)
...
# 5xFAD #
#########
sample_box2 <- metadata_metabolomics %>% dplyr::filter(box == "BOX 2") # to be removed

data_5xfad <- data_sample %>% dplyr::filter(SampleID %in% sample_5xfad$SampleID) %>%
  dplyr::filter(!(SampleID %in% sample_box2$SampleID)) %>%
  dplyr::filter(!(SampleID %in% c("B5xfad_25", "B5xfad_30", "B5xfad_26", "B5xfad_28", 
                                  "B5xfad_23", "B5xfad_27", "B5xfad_22", "B5xfad_20", 
...
for (i in c("box", "host_age", "sex", "Plate",  "Colonized", "Strain")) {
  ...
  PCA_plot <- PCA_whole_scores %>%
    ggscatter(x = "PC1", y = "PC2", color = i, alpha = 0.6,
              title = paste("PCA - Brain 5xFAD", i, sep = " "),
              xlab = paste("PC1 (", round(PCA_whole$prop_expl_var$X[1]*100, digits = 1),"%)", sep = ""), 
              ylab = paste("PC2 (", round(PCA_whole$prop_expl_var$X[2]*100, digits = 1),"%)", sep = ""),
              ggtheme = theme_classic()) +
    geom_point(data = PCA_whole_scores %>% group_by((!!sym(i))) %>% 
...
PLSDA_5xfad_strain_plot <- PLSDA_5xfad_strain_scores %>%
  ggscatter(x = "comp1", y = "comp2", color = "Strain", alpha = 0.6, title = "PLSDA - Brain 5xFAD Strain",
...
PLSDA_5xfad_colonized <- mixOmics::plsda(data_5xfad_clr %>% select_at(vars(-one_of(nearZeroVar(., names = TRUE)))),
                                         PCA_whole_scores$Colonized, ncomp = 4, scale = TRUE)
...
PLSDA_5xfad_colonized_plot <- PLSDA_5xfad_colonized_scores %>%
  ggscatter(x = "comp1", y = "comp2", color = "Colonized", alpha = 0.6, title = "PLSDA - Brain 5xFAD Colonized",
...
PLSDA_5xfad_sex_plot <- PLSDA_5xfad_sex_scores %>%
  ggscatter(x = "comp1", y = "comp2", color = "sex", alpha = 0.6, title = "PLSDA - Brain 5xFAD Sex",
...
#write_csv(x = VIPs_5xfad_strain_no_sex, file = "5xfad_brain_plsda_vip_all_samples_genotype.csv")
#write_csv(x = VIPs_5xfad_colo_no_sex, file = "5xfad_brain_plsda_vip_all_samples_colonization.csv")
#write_csv(x = VIPs_5xfad_colo_strain_clean, file = "5xfad_brain_plsda_vip_all_samples_genotype_colonization_intersection.csv")
...
for (i in c("box", "host_age", "sex", "Plate", "Strain")) {
  ...
  PCA_plot <- PCA_5xfad_gf_scores %>%
    ggscatter(x = "PC1", y = "PC2", color = i, alpha = 0.6,
              title = paste("PCA - Brain 5xFAD - GF", i, sep = " "),
...
for (i in c("box", "host_age", "sex", "Plate", "Strain")) {
  ...
  PCA_plot <- PCA_5xfad_spf_scores %>%
    ggscatter(x = "PC1", y = "PC2", color = i, alpha = 0.6,
              title = paste("PCA - Brain 5xFAD SPF", i, sep = " "),
...
PLSDA_5xfad_gf_strain_plot <- PLSDA_5xfad_gf_strain_scores %>%
  ggscatter(x = "comp1", y = "comp2", color = "Strain", alpha = 0.6, title = "PLSDA - Brain 5xFAD GF - Strain",
...
PLSDA_5xfad_spf_strain_plot <- PLSDA_5xfad_spf_strain_scores %>%
  ggscatter(x = "comp1", y = "comp2", color = "Strain", alpha = 0.6, title = "PLSDA - Brain 5xFAD SPF - Strain",
...
#            as.data.frame() %>% dplyr::mutate(Model = "5xfad_spf_strain"), file = "brain_plsda_performance_5xfad.csv")
...
#pdf(file="5xfad_brain_genotype.pdf", width = 3, height = 2.5)
#strain_5xfad
#dev.off()
...
#write_csv(x = features_5xfad_mut, file = "5xfad_brain_genotype_3xtg.csv")
#write_csv(x = features_5xfad_wt, file = "5xfad_brain_genotype_wt.csv")
...
for (i in c("box", "host_age", "Plate", "Colonized", "sex")) {
  ...
  PCA_plot <- PCA_5xfad_mut_scores %>%
    ggscatter(x = "PC1", y = "PC2", color = i, alpha = 0.6,
              title = paste("PCA - Brain 5xFAD - Mut", i, sep = " "),
...
for (i in c("box", "host_age", "Plate", "Colonized", "sex")) {
  ...
  PCA_plot <- PCA_5xfad_wt_scores %>%
    ggscatter(x = "PC1", y = "PC2", color = i, alpha = 0.6,
              title = paste("PCA - Brain 5xFAD - WT", i, sep = " "),
...
PLSDA_5xfad_mut_colo_plot <- PLSDA_5xfad_mut_colo_scores %>%
  ggscatter(x = "comp1", y = "comp2", color = "Colonized", alpha = 0.6, title = "PLSDA - Brain 5xFAD Mut - Colonization",
...
PLSDA_5xfad_wt_colo_plot <- PLSDA_5xfad_wt_colo_scores %>%
  ggscatter(x = "comp1", y = "comp2", color = "Colonized", alpha = 0.6, title = "PLSDA - Brain 5xFAD WT - Colonization",
...
colonization_5xfad <- UpSetR::upset(fromList(list_5xfad_microbiome), nsets = 4, nintersects = 8, 
                                   point.size = 1.5, line.size = 1, text.scale = 1, keep.order = TRUE,  
...
#pdf(file="5xfad_brain_colonization.pdf", width = 3, height = 2.5)
#colonization_5xfad
#dev.off()
...
#write_csv(x = features_5xfad_spf, file = "5xfad_brain_colonization_spf.csv")
#write_csv(x = features_5xfad_gf, file = "5xfad_brain_colonization_gf.csv")
...
PLSDA_5xfad_male_strain_plot <- PLSDA_5xfad_male_strain_scores %>%
  ggscatter(x = "comp1", y = "comp2", color = "Strain", alpha = 0.6, title = "PLSDA - Brain 5xFAD Male - Strain",
...
PLSDA_5xfad_male_colo_plot <- PLSDA_5xfad_male_colo_scores %>%
  ggscatter(x = "comp1", y = "comp2", color = "Colonized", alpha = 0.6, title = "PLSDA - Brain 5xFAD Male - Colonized",
...
PLSDA_5xfad_female_strain_plot <- PLSDA_5xfad_female_strain_scores %>%
  ggscatter(x = "comp1", y = "comp2", color = "Strain", alpha = 0.6, title = "PLSDA - Brain 5xFAD Female - Strain",
...
#pdf(file="5xfad_brain_upset_genotype_colonization.pdf", width = 6.5, height = 4.5)
#intersection_5xfad
#dev.off()
...
# Extract feature of interest and check difference between strains colonized
exclusive_5xfad_brain <-  rbind(combined_unique_5xfad_5xfad_spf, combined_unique_5xfad_wt_spf)
...
plot_ratio_brain_5xfad <- data_5xfad_vip_spf %>% 
  dplyr::filter(Colonized == "SPF") %>% 
  dplyr::mutate(Strain = factor(Strain, levels = c("WT", "Mut"))) %>%
  ggboxplot(y = "Ratio", x = "Strain", add = "jitter", ylab = "Ln(WT/Mut)",
            add.params = list(color = "Strain", alpha = 0.6), legend = "none",
...
#ggsave(plot = plot_ratio_brain_5xfad, filename = "5xfad_brain_ratio.svg", device = "svg", dpi = "retina", width = 1.5, height = 2.5)
#write_csv(x = data_5xfad_vip_spf, file = "brain_ratio_5xfad.csv")
...
# Extract for MN
brain_5xfad_interest <- VIPs_5xfad_spf_strain_Load %>% 
  dplyr::select(ID, comp1, GroupContrib) %>% 
  ...
  left_join(VIPs_5xfad_wt_colo_Load %>% 
              dplyr::select(ID, comp1, GroupContrib), by = c("ID" = "ID")) %>% 
  dplyr::mutate(VIP_mean = rowMeans(dplyr::select(., comp1.x, comp1.x.x, comp1.y, comp1.y.y), na.rm = TRUE)) %>%
  left_join(exclusive_5xfad_brain %>% dplyr::select(ID, Group)) %>% arrange(desc(VIP_mean))

colnames(brain_5xfad_interest) <- c("ID", "VIP1", "SPF_genotype", "VIP2", "GF_genotype",
                                    "VIP3", "5xFAD_colonization", "VIP4", "WT_colonization", 
                                    "VIP_mean", "Exclusivity")

brain_5xfad_interest <- brain_5xfad_interest %>%   
  dplyr::mutate(Genotype = case_when(SPF_genotype == GF_genotype ~ "Yes",
                                     TRUE ~ "No")) %>%
...
#write_csv(x = brain_5xfad_interest, file = "5xfad_brain_features_info_mn.csv")
...
data_3xtg_join <- data_3xtg %>%
  ...
  dplyr::mutate(sample_name = gsub(".Brain", "", sample_name)) %>% dplyr::select(-SampleID)

#write_csv(x = data_3xtg_join, file = "brain_3xtg_metabolomics_join.csv")
...
#write_csv(x = data_5xfad_join, file = "brain_5xfad_metabolomics_join.csv")


# Export all VIPs tables to be combined with the other tissues
#write_csv(x = VIPs_3xtg_strain_Load, file = "Brain_3xTG_VIPs_Strain.csv")
#write_csv(x = VIPs_3xtg_colo_Load, file = "Brain_3xTG_VIPs_Colonization.csv")
#write_csv(x = VIPs_3xtg_sex_Load, file = "Brain_3xTG_VIPs_Sex.csv")
#write_csv(x = VIPs_5xfad_strain_Load, file = "Brain_5xFAD_VIPs_Strain.csv")
#write_csv(x = VIPs_5xfad_colo_Load, file = "Brain_5xFAD_VIPs_Colonization.csv")
#write_csv(x = VIPs_5xfad_sex_Load, file = "Brain_5xFAD_VIPs_Sex.csv")
...
dda <- Spectra("data/mice_caltech/brain/gnps_brain.mgf", source = MsBackendMgf())
...
#export(dda_filtered, MsBackendMgf(), file = "Brain_VIPs_microbemasst.mgf", exportTitle = FALSE)
